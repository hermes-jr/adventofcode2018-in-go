...
func main() {
	...
	for scanner.Scan() {
		...
		match := re.FindStringSubmatch(unparsed)
		id, _ := strconv.Atoi(match[1])
		xc, _ := strconv.Atoi(match[2])
		yc, _ := strconv.Atoi(match[3])
		...
		for x := xc; x < xc+xs; x++ {
			for y := yc; y < yc+ys; y++ {
				if field[point{x: x, y: y}] == nil {
					field[point{x: x, y: y}] = list.New()
				}
				field[point{x: x, y: y}].PushBack(id)
			}
		}
		//rtm = int(math.Max(float64(rtm), float64(xc+xs)))
		//btm = int(math.Max(float64(btm), float64(yc+ys)))
	}
